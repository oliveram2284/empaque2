...
<section class="card">
  <div class="card-header">
    <span class="cui-utils-title">
      ...
      <a href="{{route('areas.create')}}" class="btn btn-primary pull-right" >Nueva Área</a>
  </div>
  <div class="card-body">
    <div class="row">
      <div class="col-lg-12">        
          ...
          <table class="table table-hover table-sm nowrap " id="table_users">
            ...
            <tbody>
                @foreach ($areas as $item)
                    <tr>
                        <td class="text-center">{{$item->id_area}}</td>
                        <td>{{$item->codigo}}</td>
                        <td>{{$item->nombre}}</td>
                        <td class="text-center">
                            <a class="view_pass_bt btn btn-sm btn-icon btn-primary mr-2" href="{{ route('areas.show',$item->id_area) }}" >
                                <i class="icmn-eye" aria-hidden="true"></i>                            
                            </a>

                            <a class="edit_bt btn btn-sm btn-icon btn-success mr-2" href="{{ route('areas.edit',$item->id_area) }}"  >
                                <i class="icmn-pencil" aria-hidden="true"></i>                            
                            </a>                        
                         

                            <a class="delete_bt btn btn-sm btn-icon btn-danger mr-2 "  href="{{ route('areas.destroy',$item->id_area) }}">
